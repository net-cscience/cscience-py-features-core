...
class TestSpatialFeature(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        tilings: List[str] = ["4_3_20percent"]
        masking_modes: List[str] = [MaskingMode.EXTRACT]
        preprocessing_modes: List[str] = [ImagePreprocessingOrder.LATE_PREPROCESSING]
        scoring_functions: List[str] = [ScoringFunction.RELATIVE_POSITIVE_NORMALIZED]
    ...
    def test_single_vs_batch(self) -> None:
        batch = self._images_repo.query("640x480")[0:10]
        batch_size = len(batch)
        for key, connector in self.connector_repository.items():
            for ic in range(batch_size):

                v = connector.image_regions(batch[ic])
                vb = connector.image_region_batch(batch)
                _, regions_per_item_v = (v.layout.item_count, vb.layout.regions_per_item)
                item_count, regions_per_item = (vb.layout.item_count, vb.layout.regions_per_item)
                self.assertEqual(regions_per_item_v, regions_per_item)

                eps = float(np.finfo(np.float32).eps)
                for jc in range(regions_per_item):
                    np.testing.assert_allclose(
                        v.vectors[jc],
                        vb.vectors[regions_per_item * ic + jc],
                        rtol=0,
                        atol=10 * eps,
                    )
